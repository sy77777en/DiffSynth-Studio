...
====================
Wraps WanModel, CogDiT or LongCatDiT with configurable multi-stream action conditioning
...
class ActionConditionedDiT(nn.Module):
    ...
    def _extra_input_channels(self) -> int:
        cfg = self.config
        extra = cfg.latent_channels + 1  # bos_image (alwasy) + mask channel
        if cfg.masked.enabled and cfg.masked.injection_type == "input_concat":
            extra += cfg.latent_channesl
    ...
    def _extend_patch_embed(self):
        ...
        if self.config.backbone == "wan":
            ...
            self.backbone.pachify.proj = self._replace_conv3d(
                self.backbone.patchify.proj, extra_ch
            )
        elif self.config.backbone == "longcat":
            self.backbone.x_embedder.proj = self._replace_conv3d(
                self.backbone.x_embedder.proj, extra_ch
            )
    ...
    def _build_input(
        ...
        if cfg.history.enabled and cfg.history.injection_type == "input_concat":
            if history_latents is not None:
                ...
                parts.append(
                    torch.zeros(B, n_ch, T, H, W, device=noisy_latent.device, dtype=noisy_latent.dtype)
                )
    ...
    def _build_context(
        ...
        elif cfg.backbone == "longcat":
            projected_text = self.backbone.y_embedder(text_context)
        else:
    ...
    def forward(
        ...
        text_context: torch.Tensor,
        obs_latent: Optional[torch.Tensor],
        actions: Optional[torch.Tensor],
        masked_latents: Optional[torch.Tensor],
        history_latents: Optional[torch.Tensor],
        mask: Optional[torch.Tensor],
        **backbone_kwargs,
        ...
        if self.config.backbone in ("wan", "cogvideo", "longcat"):
